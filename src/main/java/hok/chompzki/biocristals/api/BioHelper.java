	...
	private static int storePartialItemStack(ItemStack stack, IInventory inventory){
        ...
        else
        {
            ...
            else
            {
                if (inventory.getStackInSlot(j) == null)
                {
                	inventory.setInventorySlotContents(j, new ItemStack(item, 0, stack.getItemDamage()));

                    if (stack.hasTagCompound())
                    {
                    	inventory.getStackInSlot(j).setTagCompound((NBTTagCompound)stack.getTagCompound().copy());
                    }
                }
