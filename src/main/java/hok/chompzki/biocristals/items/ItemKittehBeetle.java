...
public class ItemKittehBeetle extends Item implements IInsect{
	...
	public void tileUpdate(TileEntity entity, ItemStack stack) {
		...
		if(BiomeRegistry.kittehsBiomes.containsKey(type)){
			...
			if(result == null)
				continue;
			BioHelper.addItemStackToInventory(result, nest, 1, 4);
		}
		...
		if(!has){
			...
			nest.workStack = null;
			nest.startTime = -1;
			BioHelper.addItemStackToInventory(new ItemStack(ItemRegistry.kittehBeetle), nest, 1, 4);
		}
	}
