...
public class BoldParseNode extends ParseNode {
	...
	public BoldParseNode() {
		super("BOLD", BoldParseNode.class);
	...
	public void parse(Cursor cursor, Article arc, TokenCollection collection) {
		...
		while(!collection.stack.isEmpty()){
			if(collection.stack.peek() == BookTokenizer.startCommand){
				...
				if(current != null){
					list.add((char)167 + "l" + current.trim() + (char)167 + "r");
				}
				return;
			}
			...
			if(token == BookTokenizer.lineEnd){
				...
				if(Article.maxWidth < cursor.x){
					...
					list.add((char)167 + "l" + current.substring(0, j).trim() + (char)167 + "r");
