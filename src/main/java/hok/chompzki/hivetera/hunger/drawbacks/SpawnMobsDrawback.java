...
public class SpawnMobsDrawback extends Drawback {
	
	private static Random rand = new Random();
	
	private final EnumResource res;
	private final double value;
	private final Class<? extends EntityMob> c;
	private final int min;
	private final int max;
	private final int radius;
	
	public SpawnMobsDrawback(EnumResource res, double value, Class<? extends EntityMob> c, int min, int max, int radius){
		this.res = res;
		this.value = value;
		this.c = c;
		this.min = min;
	...
	public void act(ResourcePackage pack, World world, int x, int y, int z) {
		...
		for(int i = 0; i < number; i++){
			...
			try {
				EntityMob mob = c.getDeclaredConstructor(c).newInstance(world);
