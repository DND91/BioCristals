...
public class ItemGreenBlower extends ItemInsect implements IArmorInsect {
	
	private final static Random rand = new Random();
	public final static String NAME = "itemGreenBlower";
	
	public ItemGreenBlower(){
		super(EnumResource.WASTE, 5.0D, 5.0D, false);
	...
	public void onArmorTick(World world, EntityPlayer player,
		...
		if(!nbt.hasKey("HIVE_JUMP"))
			return;
		if(player.isSprinting() && nbt.getBoolean("HIVE_JUMP")){
			...
			if(insect.getCost(stack) <= currentFood){
				...
				insect.setFood(stack, currentFood);
				world.createExplosion(player, player.posX, player.posY, player.posZ, 2.5F, false);
